class DependencyObject {
    ...
    DependencyObject([string]$Name, [string]$Type) {
        $this.Guid = New-Guid
        $this.Type = $Type
        $this.Name = $Name
        }
    DependencyObject([string]$Name, [string]$Type, [DependencyObject[]]$DependOn) {
        $this.Guid = New-Guid
        $this.Type = $Type
        $this.Name = $Name
        $this.DependOn = ( & {
    ...
    AddDependOn ([DependencyObject[]]$DependOn) {
        $this.DependOn = ( & {
    ...
    RemoveDependOn ([DependencyObject[]]$DependOn) {
    $this.DependOn = ( & {
        $arrDependOn = @($this.DependOn)
        foreach ($Dependency in $DependOn) {
    ...
    AddLink ([DependencyObject[]]$DependencyObject) {
            ...
            foreach ($Link in $DependencyObject) {
                $arrLinks += $Link.Guid
    ...
    RemoveLink ([DependencyObject[]]$DependencyObject) {
...
[switch]$CreateVariable
    ...
    $obj = New-Object DependencyObject -ArgumentList $name,$type,$DependOn
    if ($value) {
        $obj.Value = $value
        }
    if ($SubType) {
    ...
    if ($CreateVariable) {
        New-Variable -Name ('DOAuto_' + $obj.guid) -Value $Obj -Scope Global
        }
        return $obj
    }
...
param ()
    process {
        Get-Variable | ? Value | ? {($_.Value.GetType()).Name -eq 'DependencyObject'} | % {$_.Value}
        }
}
...
param ()
       ...
       Get-Variable -Scope Global | ? Value | ? {($_.Value.GetType()).Name -eq 'DependencyObject'} | %{Remove-Variable $_.name -Scope global}
        }
}

function Walk-DependencyObject {
        ...
        if ($dependency) {
            ...
            if ($dependency.$direction -and (-not $shouldNotContinue)) {
                    ...
                    $InvokeObject.object = $dependency
                        ...
                        Walk-DependencyObject -InvokeObject $InvokeObject
                    }
...
process {
    foreach ($Link in $DependencyObject.Links) {
...
param (
    ...
    $types | ? {$_.type -eq $DependencyObject.Type} | % {$_.parent}
    }
...
param (
    ...
    begin {
    $TypeParents = Get-DOTypeParents $DependencyObject
    }
    end {
        #[bool]$ShouldContinu
        foreach ($depth in 1..$level) {
            $parents = $DependencyObject | Walk-DependencyObject -DependOn -depth $depth 
            ...
            if ($parents.type -contains $TypeParents.Type) {
                ...
                $neighbors = $parents | Select-Object -Last 1 | Walk-DependencyObject -depth 1 -type $TypeParents.type
                if ($neighbors) {
                    $PSCmdlet.MyInvocation.MyCommand.Name + `
